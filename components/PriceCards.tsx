...
export function PriceCards() {
          ...
          .map(coin => {
            if (
              ...
              return {
                ...
                change: `${(coin.price_change_percentage_24h as number).toFixed(2)}%`,
                changeDirection:
  ...
  // Add fallback for when no data is available
  if (!threeTopCurrencies.length) {
    return (
      <div className="px-5 pt-5 text-red-500">
    ...
    <div className="grid gap-4 md:grid-cols-3 px-5 pt-5">
      {threeTopCurrencies?.map((crypto: PriceCardsData, index: number) => (
        ...
        >
          ...
          <div
            className={`flex items-center gap-1 ${crypto.changeDirection === 'up' ? 'text-green-500' : 'text-red-500'}`}
          >
